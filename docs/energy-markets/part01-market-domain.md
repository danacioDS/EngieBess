# M1 — Market Domain and Conventions

**Version:** 1.2 — Development Draft
**Status:** Under Engineering Development — Not Frozen
**Date:** 2026-09-22
**Parent Document:** Market Engineering Model — Introduction Document (v1.1)
**Upstream Dependency:** BESS Engineering Part 1 — Fundamentals and Conventions (version pinned per M1-O13)
**Document ID:** ME-M1-001 *(stable across versions)*
...
**Change control note:** The canonical lifecycle, the optional-node rule, the null-commitment path rule, and the M2 template reference rule are owned by the **Introduction Document v1.1**. M1 cites them and treats the corresponding text in §8, §10, and §27 as **informative**, not normative. Where M1 and the Introduction differ, the Introduction governs.

---

## 1. Purpose and Scope

M1 establishes the **domain-level semantic foundation** for Market Engineering.
...
M1 does **not** define individual market products in detail, product-specific eligibility rules, bidding algorithms, dispatch optimization, or settlement calculations. Those responsibilities belong to M2–M4.

M1 establishes the foundation required for those Parts to be developed consistently.

The primary question of M1 is:
...
M1 operates between the project-wide BESS foundation and the detailed Market Engineering Parts.
...
M1 consumes the project-wide conventions defined by BESS Engineering Part 1 and adds only the **market-domain semantics** required by Market Engineering.

M1 therefore does not redefine:
...
Market-specific symbols required by M1–M4 shall be registered in the **existing BESS Engineering Part 1 master symbol registry**.

**Pinned dependency:** M1 inherits from a specific, versioned release of BESS Engineering Part 1. The pin is recorded in the document header and updated only through project change control. The pin is an open item (M1-O13).
...
M1 represents the market environment as a structured engineering domain containing:
...
M1 provides the domain foundation for:

- Market Engineering Parts M2–M4;
- BESS Engineering;
...
**Physical constraints:** Physical BESS constraints (power, energy, SOC, degradation) are **not** owned by Market Engineering. They are consumed from BESS Engineering and represented in M3 only where they affect market participation or operational obligations.
...
M1 owns the definition of participant roles.
...
The market environment shall be explicitly bounded before M2–M4 are frozen.
...
M1 shall explicitly determine whether the model represents:
...
**Normative status:** The canonical chain in **Introduction v1.1** — *Product → Rules → Eligibility → Opportunity → Commitment → Dispatch → Delivery → Settlement* — is the normative semantic dependency. This diagram is an **expanded, informative view** that makes explicit the *optional* nodes and the three participation branches:
...
**Optional-node rule (Introduction v1.1):** Bid/Nomination and Award are **conditional**. They are present only where the applicable market mechanism requires them.

**Null-commitment path rule (Introduction v1.1; informative here):** On the null-commitment path:
...
- **Ownership:** M3 owns the null-path operational obligations (if any); M4 owns the null-path settlement definition.

This semantic structure is the basis for M2–M4.

---

## 9. Market Product, Rule, and Eligibility Semantics

M1 establishes the distinction between three concepts that shall not be conflated.
...
M2 owns the detailed product and eligibility representation.

M3 owns detailed operational rules and commitments.

**Boundary rule (M2/M3):** A rule is owned by **M2** if it is *specific to a product's participation or qualification* (e.g., minimum duration for a specific ancillary product). A rule is owned by **M3** if it applies *transversally to participation, operation, commitment, or constraints* (e.g., gate closure, stacking rules, operational obligations).

---

## 10. Opportunity and Commitment Semantics

M1 establishes the domain distinction between an **opportunity** and a **commitment**.
...
The exact mechanism is market-specific and shall be defined in M2/M3.
...
**Null commitment:** Where a market mechanism creates no obligation, the model represents a **null commitment**. This is a valid state for price-responsive and uncommitted participation. The downstream consequences are defined in the **Introduction v1.1** and summarized informatively in §8.

---

## 11. Market Temporal Structure

M1 does not create a new project-wide temporal convention.
...
M1 shall represent their relationships without changing the project-wide temporal conventions.
...
**Ownership:** M1 owns the **temporal mapping framework** — how market periods relate to the project time axis and to each other. M3 owns the **actual rule values** — gate closure time, delivery period duration, settlement period duration, and their market-specific timings.
...
Where market data resolution differs from the project model resolution (e.g., 5-minute settlement data against a 15-minute model), M1 shall define:
...
**Ownership:** M1 owns the **mapping method**. M4 **declares** the authoritative settlement quantity per product. M4 owns the declared value; M1 owns the requirement that it be declared and the method by which it is derived.
...
M1 establishes the generic semantic category of a market signal.
...
M1 defines the generic signal concept.

M3 shall define the detailed signals relevant to the applicable market.

### 13.1 Terminology Rules

**Dispatch.** The term "dispatch" is reserved for two distinct concepts, which shall not be conflated:

- **Operator Dispatch Instruction** — a market signal issued by the market/system operator, owned by M3.
...
M1 distinguishes three principal sources of constraints.
...
**Ownership:** Product-specific participation and qualification constraints are owned by **M2** (per the §9 boundary rule). Transversal market constraints — those applying across products — are owned by **M3**.
...
Where they affect market participation or operational obligations, they are represented in M3 with their evidence source explicitly identified.

### 14.3 Physical Constraints

Physical BESS constraints (power, energy, SOC, degradation) are **not owned by Market Engineering**. They are consumed from BESS Engineering and represented in M3 only where they affect market participation or operational obligations.

**Ownership of the Constraint entity:** §19 assigns ownership by *type* — product-specific market constraints to M2, transversal market constraints to M3, grid/network constraints to M3 (with source evidence distinct), physical constraints to BESS Engineering.

---

## 15. Price-Influence and Market-Behavior Assumptions

M1 shall establish the market-behavior assumption applicable to the model.
...
The assumption shall be recorded as an M1 domain-level assumption and inherited by downstream Parts unless explicitly changed through project change control.
...
M1 establishes the common evidence model used throughout Market Engineering.
...
M1 establishes the ownership principle:

> **Each Part owns the uncertainty associated with the rules and concepts it defines.**

Additionally:

- M1 owns domain-scope, role, structural, and foundational assumption uncertainty;
- M2 owns product, qualification, and eligibility-rule uncertainty;
- M3 owns market-rule, signal, market-data, commitment, and operational-constraint uncertainty;
...
| Market environment | M1 |
| Participant role | M1 |
| Market party (entity model) | M1 |
| Market behavior assumption (price-influence axis) | M1 |
| Temporal mapping framework | M1 |
| Resolution mapping method | M1 |
| Opportunity (definition) | M1 |
| Signal (generic concept) | M1 |
| Market product (definition) | M2 |
| Eligibility | M2 |
| Product-specific participation / qualification rules | M2 |
| Product-specific market constraints | M2 |
| Participation mechanism | M2 |
| Opportunity (application) | M3 |
| Market rule (transversal) | M3 |
| Market signal (specific) | M3 |
| Signal (application) | M3 |
| Operator Dispatch Instruction | M3 |
| Market commitment | M3 |
| Bid / Nomination | M3 |
| Award | M3 |
| Delivery obligation (part of the commitment) | M3 |
| Stacking / coexistence | M3 |
| Transversal market constraints | M3 |
| Grid / network constraints (market-relevant) | M3 |
| Temporal rule values (gate closure, delivery timing, settlement timing) | M3 |
| Asset Dispatch Decision | Operational/Optimization Engineering |
| Delivery definition and measurement | M4 |
| Performance measurement | M4 |
| Baseline / Counterfactual | M4 |
| Settlement | M4 |
| Authoritative settlement quantity (declared per product) | M4 |
...
- **Opportunity** — M1 owns the definition; M3 owns the application.
- **Signal** — M1 owns the generic concept; M3 owns specific signals and the application.
- **Product** — M2 owns the definition; M3 owns the application in market rules.

The rule does **not** transfer eligibility ownership: eligibility remains with M2, because eligibility is defined at the product level, not at the application level.
...
M1 shall use the project-wide master symbol registry established by BESS Engineering Part 1.

M1 may identify required market-specific symbols, but shall not establish an independent registry.
...
M1 provides the common domain semantics and conventions required by:

- M2 Products and Participation;
- M3 Rules, Signals, Commitments and Constraints;
...
- actual or simulated delivered quantities.
...
- delivery quantities;
- prices or price definitions where applicable;
- penalties;
- adjustments;
- other settlement outputs.

Financial Engineering transforms these into economic consequences.

### 21.6 Software Engineering

M1 defines the semantic and structural requirements that software must represent.
...
**Change control for shared artifacts:** Changes to the canonical lifecycle, the optional-node rule, the null-commitment path rule, or the M2 product template are owned by the **Introduction Document**. Such changes shall be made in the Introduction and referenced from M1–M4. M1 does not declare them unilaterally.

---

## 23. M1 Open Items

The following items must be resolved before M1 can be frozen.

| ID | Open item | Required outcome | Owner | Priority | Blocking dependency | Target date |
| --- | --- | --- | --- | --- | --- | --- |
| M1-O01 | Jurisdiction | Applicable market jurisdiction identified | Market Engineering Lead | Critical | Blocks O02–O12 | TBD |
| M1-O02 | Market operator | Responsible market/system operator identified | Market Engineering Lead | Critical | Depends on O01 | TBD |
| M1-O03 | Wholesale / BTM | Applicable participation environment established | Market Engineering Lead | Critical | Depends on O01, O02 | TBD |
| M1-O04 | Participant roles | Actual roles mapped to domain entities | Market Engineering Lead | High | Depends on O01, O02 | TBD |
| M1-O05 | Products | Candidate product universe identified for M2 | Market Engineering Lead | High | Depends on O03 | TBD |
| M1-O06 | Market periods | Applicable bid, commitment, delivery, and settlement periods identified | Market Engineering Lead | High | Depends on O03 | TBD |
| M1-O07 | Network environment | Applicable grid/interconnection constraints identified | Market Engineering Lead | Medium | Depends on O01 | TBD |
| M1-O08 | Market behavior | Price-influence assumption established | Market Engineering Lead | High | Depends on O03 | TBD |
| M1-O09 | Evidence sources | Authoritative source set established | Market Engineering Lead | High | Depends on O01, O02 | TBD |
| M1-O10 | Data boundary | Required external market-data categories identified | Market Engineering Lead | Medium | Depends on O03, O06 | TBD |
| M1-O11 | Regulatory horizon | Relevant long-horizon rule changes identified | Market Engineering Lead | Medium | Depends on O09 | TBD |
| M1-O12 | Commercial boundary | Any required separation from contractual/commercial modeling confirmed | Market Engineering Lead | Medium | Depends on O03 | TBD |
| M1-O13 | Pinned dependency | BESS Engineering Part 1 version pinned in M1 header | BESS Engineering + Market Engineering Lead | Critical | Blocks freeze; independent of O01–O03 | TBD |

These are **engineering open items**, not assumptions to be silently resolved by the model.

**Critical path:** M1-O01 → M1-O02 → M1-O03 block nearly all other items and shall be resolved first. M1-O13 blocks freeze independently of the O01–O03 chain.

---

## 24. M1 Validation Criteria

M1 shall be validated before proceeding to a frozen M2 baseline.
...
- **Reference case:** Introduction v1.1 rules plus the downstream Parts' drafts (M2–M4). This document alone is not a sufficient reference case.
...
- **Method:** Interface review with BESS, Forecasting, Operational/Optimization, Financial, and Software Engineering.
...
- **Reference case:** BESS Engineering Part 1 (pinned version, per M1-O13).
- **Reviewer:** BESS Engineering.
- **Pass condition:** No conflicting symbols, units, state conventions, or temporal conventions.

---

## 25. M1 Acceptance Criteria

M1 is ready for engineering freeze when:
...
- interfaces with BESS, Forecasting, Operational/Optimization, Financial, and Software Engineering are defined;
- market-derived requirements can propagate upstream through change control;
- changes to Introduction-owned artifacts are made in the Introduction and referenced from M1;
...
- M1-O13 (pinned BESS Part 1 version) is resolved;
...
M1 follows the project-wide maturity ladder:
...
- **External Evidence — pending market/jurisdiction identification**
- **Engineering Validation — pending**
- **Frozen — No**

M1 should not proceed to **Frozen** until the applicable market environment and authoritative evidence base are established.

**Note on diminishing returns:** The remaining consistency issues are minor. Further abstract iteration on M1 will add little. The next real gain is **External Evidence**, which is blocked on M1-O01 through O03. Once those are resolved, instantiating the framework against real market documents will test it more effectively than another review round.

---

## 27. Output of M1

The final output of M1 shall be a controlled **Market Domain Specification** containing:
...
### M2 Product Definition Template (Referenced, Not Owned)

The M2 product template and the "reference, do not redefine" rule are **owned by the Introduction Document v1.1**. M2 shall apply them. The list below is **informative** and shall be kept in sync with the Introduction.

**M2 references, but does not redefine, concepts owned by M3 and M4.** Specifically:

- Market Signal → reference M3 identifier
- Operator Dispatch Instruction → reference M3 identifier
- Commitment → reference M3 identifier
- Delivery Obligation → reference M3 identifier
- Delivery Definition and Measurement → reference M4 identifier
- Performance Measurement → reference M4 identifier
- Settlement Mechanism → reference M4 identifier
- Interaction with Other Products (stacking/coexistence) → reference M3 identifier

M2 defines only what is product-specific: definition, market purpose, eligibility, required BESS capability, participation mechanism, temporal requirements specific to the product, and product-specific risks and uncertainties.