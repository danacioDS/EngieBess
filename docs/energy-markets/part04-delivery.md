...
**Version:** 1.1 — Development Draft
**Status:** Under Engineering Development — Not Frozen
**Date:** 2026-09-22
**Parent Document:** Market Engineering Model — Introduction Document (v1.2)
**Upstream Dependencies:**
- M1 — Market Domain and Conventions (v1.2), specifically §7, §8, §10, §11, §12, §16, §18, §19
- M2 — Market Products and Participation (v1.2), specifically §5 (Product-Mechanism), §7 (eligibility time dimension), §11 (temporal)
- M3b — Market Rules, Signals, Commitments and Constraints (v1.3), specifically §6 (signals), §7 (commitment definitions), §8 (commitment instances), §9 (obligations), §12.4 (M4 interface)
- BESS Engineering Part 1 — Fundamentals and Conventions (pinned per M1-O13)

**Scope note:** This document is **M4 — Delivery, Performance and Settlement**. It is drafted after M3b freezes. It consumes M3b's commitment definitions and delivery-obligation definitions and defines how delivery is measured, how performance is assessed and scored, how baselines and counterfactuals are established, how settlement quantities and **settlement amounts** are determined, and how market outputs are provided to downstream engineering.

**Document ID:** ME-M4-001 *(stable across versions)*
...
**Change control note:** The canonical lifecycle, the optional-node rule, the null-commitment path rule, the M2 template reference rule, and the M3 split are owned by **Introduction v1.2**. M3b owns commitment definitions, delivery obligations, signals, and constraints. M4 applies them and defines delivery, performance, and settlement. Where M4 and the Introduction differ, the Introduction governs.

**Pending change requests:**
- **Introduction v1.3** — Add Network Engineering as an engineering domain (owned by Introduction; referenced by M4 §12.9).
- **M1-O14** — Site/interconnection capability definition and value ownership (referenced by M4 §5.4 for baselines on the BTM path).
- **M3b-O17** — Award simulation rule (referenced by M4 §10.6 for simulated settlement).

---

## 1. Purpose and Scope

M4 establishes the **delivery, performance, and settlement layer** for Market Engineering.
...
- **M4 assessment records** — the assessment records linked to M3b Commitment Instances **or, on the null-commitment path, to a Product-Mechanism plus settlement window**;
- **settlement uncertainty** — the uncertainty owned by M4.
...
- market rules, signals, commitment definitions, commitment instance formation data, operational obligations, or stacking constraints (M3b);
- **raw delivered quantities** — M4 receives them from Operational/Optimization Engineering or from the meter and applies measurement rules to produce **recognized delivery quantities**;
...
- site/interconnection capability (M1 definition; pending M1-O14).

M4 defines **how delivery is measured, how performance is assessed and scored, how settlement quantities and settlement amounts are determined** — and provides the settlement amounts and supporting quantities and prices that Financial Engineering uses for economic valuation.

The primary question of M4 is:
...
M4 is drafted **after M3b freezes**. M4 consumes M3b's commitment definitions and delivery-obligation definitions.
...
- **market environment (wholesale or BTM) as a separate axis from the participation branch (M1 §7)**;
- commitment vs. dispatch vs. delivery distinction (M1 §10);
- temporal framework (M1 §11);
- time relationships (M1 §12);
- evidence model (M1 §16);
- uncertainty ownership (M1 §18);
- semantic ownership (M1 §19).

**M4 consumes from M2:**

- product identifiers and Product-Mechanism records (M2 §4, §5);
- eligibility time dimension — validity windows, evaluation basis (M2 §7.5);
- Product-Mechanism temporal requirements (M2 §11).

**M4 consumes from M3b:**

- commitment definitions (M3b §7.4);
- commitment instances, formation data only (M3b §8.2);
- delivery-obligation definitions (M3b §7.4, §4.2);
- signal definitions and timing (M3b §6);
- operational obligations (M3b §9);
- constraint definitions relevant to delivery (M3b §11);
- baseline/counterfactual trigger conditions, where market-defined (M3b §7.4).

**M4 consumes from Operational/Optimization Engineering:**

- **raw delivered quantities** (metered or simulated);
- operating trajectories;
- Asset Dispatch Decisions.

**M4 provides to Operational/Optimization Engineering:**

- delivery requirements (obligation side, by reference to M3b);
- performance feedback where the operational model simulates delivery.

**M4 provides to Financial Engineering:**
...
**M4 provides to BESS Engineering (via change control):**

- delivery-derived physical requirements, where delivery assessment reveals a physical gap.

**M4 does not create an independent symbol registry.** Market-specific **symbols** are registered in the BESS Engineering Part 1 master symbol registry (M1 §20). **Identifiers** are governed by M1's identifier scheme.

### 2.3 Ownership Boundary by Kind of Record

| Kind of record | Owner |
| --- | --- |
| Commitment definitions (formation, obligation, satisfaction condition, instance lifetime) | M3b |
| Commitment instance formation data (quantity, direction, price, period, source) | M3b |
| Commitment instance M3b states (Pending, Active, Cancelled) | M3b |
| Delivery obligation definitions (what the commitment obliges) | M3b |
| Signal definitions and timing (including publication timing) | M3b |
| Operational obligations (ongoing) | M3b |
| Raw delivered quantity | Operational/Optimization Engineering (or the meter) |
| Delivery definition and measurement | M4 |
| Recognized delivery quantity | M4 |
| Performance measurement and assessment | M4 |
| Delivery scoring thresholds and scoring rules | M4 |
| Baselines and counterfactuals | M4 |
| Deviation measurement | M4 |
| Compliance determination | M4 |
| Settlement quantities | M4 |
| Settlement price semantics | M4 |
| **Settlement amounts** | **M4** |
| Settlement adjustments and penalties | M4 |
| Authoritative settlement quantity | M4 |
| Market outputs | M4 |
| M4 assessment records (linked to M3b instances, or to `Product-Mechanism + settlement window` on the null-commitment path) | M4 |
| Commitment instance M4 states (Satisfied, Breached) | M4 |
| Settlement uncertainty | M4 |
| **Economic valuation** (discounting, tax, NPV, IRR) | **Financial Engineering** |

**Reference rule:** Where a record's threshold or definition is owned by M3b (or M2, M3a, or M1), M4 **references** it by ID. M4 does not restate it.
...
M4 represents the delivery, performance, and settlement layer as a structured set of:
...
- **M4 assessment records** — the assessment linked to each M3b Commitment Instance **or, on the null-commitment path, to a Product-Mechanism plus settlement window**;
- **Settlement uncertainty** — the uncertainty owned by M4.

### 3.2 WHY

The purpose is to prevent downstream engineering from treating delivery and settlement as a single undifferentiated calculation, and to preserve the distinctions that M1 established:
...
- **Settlement is distinct from financial valuation** (M1 §4).

### 3.3 FOR WHOM

M4 provides the delivery, performance, and settlement foundation for Financial Engineering, Operational/Optimization Engineering, Forecasting Engineering, Software Engineering, and BESS Engineering (via change control for delivery-derived physical requirements).
...
- **commitment** — the obligation (M3b);
- **Asset Dispatch Decision** — the operational decision (Operational/Optimization);
- **performance** — the comparison of delivery to the commitment or baseline (M4 §6);
- **settlement quantity** — the recognized quantity used for settlement (M4 §9);
- **settlement amount** — the currency amount produced by applying settlement price semantics (M4 §9).

### 4.2 Raw vs. Recognized Delivery Quantity

M4 distinguishes two quantities:
...
| **Recognized delivery quantity** | M4 | The delivered quantity after measurement rules (authoritative source, interval labeling, resolution mapping, correction) are applied. |

M4 receives the **raw delivered quantity** as an input and produces the **recognized delivery quantity** as the delivery record's primary field.
...
| **Interval labeling** | Interval-beginning / Interval-ending (references M1 §11.1) |
| **Time zone reference** | Reference to the M1 temporal framework instance |
...
On the null-commitment path (M1 §8, M3b §7.6):
...
- **No Commitment Instance exists** on this path. The M4 assessment record, deviation record, and penalty record link to the **Product-Mechanism and settlement window** directly.
...
> **On the null-commitment path, the M4 assessment record, deviation record, and penalty record link to the `Product-Mechanism ID + settlement window` directly. No Commitment Instance exists on this path.**

### 4.6 Delivery vs. Commitment

The distinction (M1 §10, M1 §4) shall be preserved:

- **Commitment** — the obligation, owned by M3b.
- **Delivery** — the physical or market-recognized quantity/state associated with the commitment, owned by M4.
- **Delivery obligation** — part of the commitment, owned by M3b.
- **Delivery definition and measurement** — owned by M4.

### 4.7 Authoritative Delivery Quantity

Where multiple measurement sources exist (metered, telemetry, settlement-system), M4 shall declare which is the **authoritative delivery quantity**. The authoritative quantity is used for performance assessment and settlement quantity determination.
...
- **Commitment** — what the asset is obliged to do (M3b).
- **Baseline** — what the reference scenario would have been (M4).
...
M4 owns the baseline definition and measurement. Where the baseline depends on site/interconnection capability or a site-specific reference, M4 references **M1-O14** (pending).

### 5.5 Baseline Uncertainty

Baseline uncertainty (measurement uncertainty, counterfactual estimation uncertainty, data gaps) is owned by **M4** (M1 §18).
...
- **delivery** — the delivered quantity (M4 §4);
- **settlement quantity** — the recognized quantity used for settlement (M4 §9);
- **compliance** — the determination of whether the commitment was satisfied (M4 §8).
...
| **Commitment Instance ID** | The M3b instance |
| **Delivery ID** | The delivery record |
| **Committed quantity** | The committed quantity (from M3b) |
| **Recognized delivered quantity** | The delivered quantity (from M4 §4) |
...
| **Tolerance** | The tolerance band, **owned by M4** (see §7). Where the tolerance is an obligation threshold owned by M3b, reference the M3b obligation by ID. |
| **Performance result** | Satisfied / Partially satisfied / Not satisfied / Exceeded (or scored) |
| **Owner** | M4 |
| **Uncertainty reference** | Link to **§14** |

**Tolerance ownership rule:** Tolerance is owned by **M4** unless the market defines it as an operational obligation, in which case **M3b** owns it and M4 references it by ID.
...
Where it applies, the performance measurement compares **recognized delivery** to the **baseline** (M4 §5) rather than to a commitment.
...
On the BTM path, performance measurement compares **recognized delivery** to the **baseline** (M4 §5.2). The baseline is the counterfactual consumption or production.

### 6.5 Environment vs. Branch

The **participation branch** (1, 2, or 3, per M1 §8) determines the **commitment semantics**. The **market environment** (wholesale or BTM, per M1 §7) determines **whether delivery is assessed against a baseline**.
...
**Delivery scoring** (M4 §7) assigns a score or penalty based on the degree of satisfaction.
...
Delivery scoring thresholds are owned by **M4** (M3b §9.2, §2.3).
...
Where deviation is material (per **M1 §16.4** materiality), a deviation record shall be created:
...
M4 owns the **Satisfied** and **Breached** states of the M3b Commitment Instance (M3b §8.3). The compliance record is the M4-side record that determines those states.

**Mapping (normative):**

| M4 compliance state | M3b instance state | Notes |
| --- | --- | --- |
...
> **Whether over-delivery (Exceeded) is penalized is a market-defined rule, referenced from M3b §9 or defined as an M4 penalty rule. Where the market penalizes over-delivery, the penalty is recorded in the penalty record (§10.4).**
...
Settlement is **distinct from financial valuation** (M1 §4). M4 determines the settlement quantities, settlement price semantics, and **settlement amounts**. Financial Engineering determines the economic valuation of those amounts.
...
- the commitment quantity (M3b);
- the recognized delivery quantity (M4 §4);
- the deviation (M4 §8);
- the score (M4 §7), where applicable;
- the applicable settlement rule.

### 9.3 Authoritative Settlement Quantity

Where multiple quantities are available (committed, delivered, metered, telemetry-derived), M4 shall declare which is the **authoritative settlement quantity**.

**The authoritative settlement quantity is declared per Product-Mechanism** and shall be recorded explicitly.

The declaration is **owned by M4** (M2 §11.3, M1 §11.3).

### 9.4 Settlement Price Semantics

M4 defines **how settlement prices are applied**. It does **not** define the prices themselves where they are market signals (M3b §6).

Settlement price semantics include:

- which signal (M3b §6) is applied;
...
M4 computes the **settlement amount** by applying the settlement price semantics to the settlement quantity.
...
On the BTM path, settlement quantities are determined from the **recognized delivery quantity** and the **baseline** (M4 §5.2), settled at the applicable tariff or price. The settlement amount is computed per §9.7.
...
Penalties are **settlement inputs** owned by M4. Their economic valuation is Financial Engineering's.
...
Deviation does not automatically produce a penalty. The penalty rule is market-defined and owned by M4.
...
- simulated awards and commitments (governed by M3b-O17, the Award Simulation Rule);
...
## 11. M4 Assessment Record

### 11.1 Purpose

The **M4 Assessment Record** is the record linked to each M3b Commitment Instance — **or, on the null-commitment path, to a Product-Mechanism plus settlement window** — that holds the M4-owned assessment data: recognized delivery quantity, satisfaction status, compliance state, settlement quantities, settlement prices, and settlement amounts.
...
| **M4 state** | Satisfied / Breached (M4-owned states of the M3b instance; empty on the null-commitment path) |
| **Assessment date** | When the assessment was performed |
| **Owner** | M4 |
| **Uncertainty reference** | Link to **§14** |

### 11.3 M4 State Ownership

M4 owns the **Satisfied** and **Breached** states of the M3b Commitment Instance (M3b §8.3). M3b owns **Pending**, **Active**, and **Cancelled**.

On the **null-commitment path**, no M3b instance exists, so the M4 state field is empty; the assessment record links to the `Product-Mechanism ID + settlement window` directly.

### 11.4 Assessment Record Consumers

The M4 Assessment Record is consumed by:
...
**Consumes:** transversal values relevant to delivery and settlement timing, where applicable.

**Provides:** feedback on transversal values encountered during delivery and settlement definition.
...
### 12.4 M3b — Market Rules, Signals, Commitments and Constraints
...
**Provides:** M4 assessment records linked to Commitment Instances **or to `Product-Mechanism + settlement window` on the null-commitment path**; satisfaction status; settlement definitions referenced from commitment definitions; delivery measurement; delivery scoring.
...
**Consumes:** delivery requirements (obligation side, by reference to M3b); performance feedback.

**Provides:** **raw delivered quantities** (metered or simulated); operating trajectories; Asset Dispatch Decisions.

**Semantic boundary:** *Market defines the commitment and its requirements; Operational/Optimization defines the Asset Dispatch Decision and the raw delivered quantities. M4 defines how delivery is recognized and settled.*
...
**Semantic boundary:** *M4 determines settlement quantities, settlement price semantics, and settlement amounts. Financial Engineering determines the economic valuation of those amounts (discounting, tax, NPV, IRR).*

### 12.8 Software Engineering

**Provides:** delivery record structure; baseline record structure; performance measurement structure; delivery scoring structure; deviation and compliance record structures; settlement quantity, price, and amount structures; adjustment and penalty structures; M4 assessment record structure; market output structure.

### 12.9 Network Engineering

**Network Engineering** is defined in the Introduction (v1.3). M4 references it where delivery assessment depends on network-side evidence (e.g., curtailment events, interconnection constraints affecting delivery).

**Pending Introduction v1.3:** Until Introduction v1.3 is issued, M4 refers to the Network Operator role and BESS Engineering for network-side delivery evidence.
...
A **market output** is a quantity, price, amount, or status provided by M4 to downstream engineering.
...
| **Assessment records** | M4 assessment records | Financial Engineering, Software Engineering |
...
| **Source record** | The M4 record that produced the output |
| **Product-Mechanism ID** | The Product-Mechanism |
| **Linkage** | **Either** `Commitment Instance ID` **or** `Product-Mechanism ID + settlement window` |
...
M4 does not perform financial valuation.
...
Uncertainty types shall not be conflated (M1 §18).
...
Regulatory and market-rule changes over the asset life are **long-horizon uncertainty** (M1 §18). M4 shall record such changes with effective dates on the evidence records (M1 §16.2).

### 14.3 Uncertainty vs. Forecast Uncertainty

Settlement-rule uncertainty (M4) is **distinct from** forecast uncertainty (Forecasting Engineering). An uncertain forecast does not imply an uncertain settlement rule (M1 §18).
...
| --- | --- | --- | --- | --- | --- | --- |
| M4-O01 | Delivery definitions | Delivery defined per Product-Mechanism | M4 Lead | Critical | Depends on M3b freeze | TBD |
| M4-O02 | Authoritative delivery quantity | Authoritative delivery quantity declared per Product-Mechanism | M4 Lead | High | Depends on M4-O01 | TBD |
| M4-O03 | Baselines | Baselines defined per applicable Product-Mechanism | M4 Lead | High | Depends on M4-O01 | TBD |
| M4-O04 | Performance measurement | Performance measurement defined per Product-Mechanism | M4 Lead | High | Depends on M4-O01 | TBD |
| M4-O05 | Delivery scoring | Delivery scoring rules and thresholds defined | M4 Lead | High | Depends on M4-O04 | TBD |
| M4-O06 | Deviation and compliance | Deviation and compliance determination defined; compliance-to-instance mapping applied | M4 Lead | High | Depends on M4-O04 | TBD |
| M4-O07 | Settlement quantities | Settlement quantities defined per Product-Mechanism | M4 Lead | Critical | Depends on M4-O01, M4-O04 | TBD |
| M4-O08 | Authoritative settlement quantity | Authoritative settlement quantity declared per Product-Mechanism | M4 Lead | Critical | Depends on M4-O07 | TBD |
| M4-O09 | Settlement price semantics | Settlement price semantics defined per Product-Mechanism, including real vs. simulated sources | M4 Lead | High | Depends on M4-O07 | TBD |
| M4-O10 | Settlement amounts | Settlement amount rule defined per Product-Mechanism; currency handling defined | M4 Lead | Critical | Depends on M4-O07, M4-O09 | TBD |
| M4-O11 | Adjustments and penalties | Adjustment and penalty rules defined per Product-Mechanism | M4 Lead | High | Depends on M4-O06 | TBD |
| M4-O12 | M4 assessment records | Assessment record structure defined and integrated with M3b instances and null-path linkages | M4 Lead | High | Depends on M4-O07 | TBD |
| M4-O13 | Market outputs | Market output structure defined | M4 Lead | High | Depends on M4-O07, M4-O09, M4-O10, M4-O11 | TBD |
| **M4-O14** | **Simulation confirmation** | **Confirm settlement rules apply unchanged to simulated inputs. Justify any deviation.** | **M4 Lead + Op/Opt Lead** | **Critical** | **Depends on M3b-O17; blocks energy pilot** | **TBD** |
| M4-O15 | Uncertainty records | Uncertainty records defined per delivery, performance, baseline, and settlement | M4 Lead | Medium | Depends on M4-O01–M4-O11 | TBD |
| M4-O16 | Network-side delivery evidence | Network-side delivery evidence interface established (pending Introduction v1.3) | M4 Lead + Network Engineering | Medium | Depends on Introduction v1.3 | TBD |
| M4-O17 | Null-path linkage confirmation | Confirm M3b does not define null instances; M4 keys on `Product-Mechanism + settlement window` on the null-commitment path | M4 Lead + M3 Lead | High | Depends on M3b freeze | TBD |
| **M4-O18** | **Settlement statement ownership** | **Confirm M4 computes the settlement amount. Define currency handling and the settlement amount record.** | **M4 Lead + Financial Engineering** | **Critical** | **Blocks energy pilot** | **TBD** |

**Critical path:** M4-O01 (delivery definitions) blocks most items. M4-O07 (settlement quantities), M4-O08 (authoritative settlement quantity), and M4-O10 (settlement amounts) are critical for Financial Engineering. **M4-O14 (simulation confirmation) and M4-O18 (settlement statement ownership) are critical for the energy pilot.**

---

## 16. M4 Validation Criteria
...
- **Reference case:** Market documentation for the applicable jurisdiction; M3b commitment definitions.
- **Reviewer:** M4 Lead + independent reviewer.
...
- **Reference case:** Market documentation; site/interconnection capability (pending M1-O14).
- **Reviewer:** M4 Lead + Network Engineering (where network-side data is used).
...
- **Reference case:** M3b commitment definitions; M4 baseline definitions.
- **Reviewer:** M4 Lead + Operational/Optimization Engineering.
...
- **Reviewer:** M4 Lead.
- **Pass condition:** Each scoring rule is correctly defined; thresholds are owned by M4; scores are correctly mapped to settlement inputs.
...
- **Reference case:** Market documentation.
- **Reviewer:** M4 Lead.
- **Pass condition:** Deviation is correctly measured; compliance states are correctly determined; **the compliance-to-instance mapping is correct**; the M4-owned Satisfied and Breached states are correctly linked to M3b instances; the over-delivery rule is applied.
...
- **Reference case:** Market documentation; M3b commitment definitions; M4 delivery and performance definitions.
- **Reviewer:** M4 Lead + independent reviewer + Financial Engineering.
...
- **Reviewer:** M4 Lead + Financial Engineering.
...
- **Method:** Review of the M4 assessment record structure and its integration with M3b instances **and null-path linkages**.
- **Reference case:** M3b commitment instance structure; M4 §4.5 null-path linkage rule.
- **Reviewer:** M4 Lead + M3 Lead.
- **Pass condition:** Each assessment record links correctly to its M3b instance **or to a `Product-Mechanism + settlement window` on the null-commitment path**; M4-owned states are correctly determined; M3b-owned states are not redefined.

### Boundary Validation

- **Method:** Interface review with M1, M3a, M2, M3b, Operational/Optimization, Forecasting, Financial, Software, and Network Engineering.
- **Reference case:** Interface definitions in §12.
- **Reviewer:** Cross-domain review board.
- **Pass condition:** No responsibility is claimed by two domains; no responsibility is unowned; M4 does not restate M3b definitions; M4 does not perform financial valuation; **raw vs. recognized delivery is preserved**; **the environment-vs-branch distinction is preserved**.
...
- **Reference case:** BESS Engineering Part 1 (pinned version), M1 v1.2, M3a, M2 v1.2, M3b v1.3, Introduction v1.2 (and v1.3 once issued).
- **Reviewer:** BESS Engineering + M4 Lead.
- **Pass condition:** No conflicting symbols, identifiers, units, state conventions, or temporal conventions; no M3b, M2, or M1 concept is redefined in M4; **all uncertainty references resolve to §14**; **all internal cross-references resolve correctly**; no M2 capability mapping is consumed by M4.

---

## 17. M4 Acceptance Criteria

M4 is ready for engineering freeze when:
...
- delivery scoring rules and thresholds are defined, with thresholds owned by M4;
...
- M4-owned Satisfied and Breached states are correctly linked to M3b instances; **null-commitment path assessments link to `Product-Mechanism + settlement window`**;
...
- M4 assessment records are defined and integrated with M3b instances and null-path linkages;
...
- **M4-O14 (simulation confirmation) is resolved**;
- **M4-O18 (settlement statement ownership) is resolved**;
- M1-O14 is resolved, where baselines depend on site/interconnection capability;
...
- interfaces with M1, M3a, M2, M3b, Operational/Optimization, Forecasting, Financial, Software, and Network Engineering are defined;
...
M4 follows the project-wide maturity ladder:
...
- **Internal Consistency — ready for validation** *(revised from v1.0; the null-path linkage, the compliance mapping, the settlement amount, and the environment-vs-branch distinction are now explicit)*
- **External Evidence — pending M3b freeze and market documentation**
- **Engineering Validation — pending**
- **Frozen — No**

**Status note on sequencing.** M4 is drafted after M3b freezes. M3b is out of sequence by its own rules (M3b should be drafted after M2 freezes, which requires M3a). **The next engineering activity is M3a, not M4.**

**Status note on structural gaps.** Four structural items are pending:

- **M1-O14** — site/interconnection capability, including value ownership.
- **Introduction v1.3** — Network Engineering as an engineering domain.
- **M3b-O17** — award simulation rule. Required before the energy pilot.
- **M4-O17** — null-path linkage confirmation with M3b.
- **M4-O18** — settlement statement ownership.

**Status note on diminishing returns.** The M4 framework is complete as of v1.1. The remaining work is populating delivery, baseline, performance, scoring, deviation, compliance, settlement, adjustment, and penalty definitions from authoritative market documentation. Further abstract iteration on the framework will add little. The next real gain is **External Evidence**, which is blocked on M3a and M3b freeze.

---

## 19. Output of M4

The final output of M4 shall be a controlled **Market Delivery, Performance and Settlement Specification** containing:
...
14. M4 assessment record structure and integration with M3b instances and null-path linkages;
15. market outputs and their structure;
16. uncertainty records per delivery, performance, baseline, and settlement;
17. simulation confirmation (M4-O14);
18. settlement statement ownership confirmation (M4-O18);
19. null-path linkage confirmation (M4-O17);
...
> **Financial Engineering:** What is the economic value of the market outcomes determined by M4?
>
...
- **Participation branch** (M1 §8) — determines **commitment semantics**;
- **Market environment** (M1 §7) — determines **whether delivery is assessed against a baseline**.
...
**On the null-commitment path**, the M4 assessment record, deviation record, and penalty record link to `Product-Mechanism ID + settlement window` directly, because no Commitment Instance exists.
